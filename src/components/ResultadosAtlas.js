...
export default class ResultadosAtlas extends Component {
    ...
    async componentDidMount() {
        ...
        const res3 = await axios.get(
            'http://167.99.15.83:4000/api/atlas/productos/' + this.props.codeResultado
        );
    ...
    render() {
        return ( <
            ...
            br / >
            <
            hr / > { /* Hover table card start */ } <
            div className = "card" >
            <
            div className = "card-header" >
            <
            h4 >
            Productos del Resultado({ this.props.codeResultado }): { ' ' } { this.props.nameResultado } { ' ' } <
            /h4> <
            div >
            <
            button type = "button"
            ...
            tbody > {
                this.state.productos_atlas.map((Producto) => ( <
                    ...
                    td > { Producto.name } < /td> <
                    td style = {
                        { whiteSpace: 'normal' } } > { Producto.details } <
                    /td> <
...

            {
                this.state.productos_atlas.map((result) => ( <
                    AtlasActividades className = "mt-3"
