...
export default class TableCost extends Component {
  ...
  async componentDidMount() {
    ...
    const res = await axios.post(
      "http://167.99.15.83/api/budgetlines/project/" + this.props.idProject
    );
  ...
  onClickAprobar = async (id, maximo) => {
    ...
    if (this.state.valor <= maximo) {
      ...
      await axios.post(
        "http://167.99.15.83/api/budgetlines/aprobar/" +
        id +
        "/" +
        this.state.aprobar +
        "/" +
  ...
  onSubmit = async (e) => {
    e.preventDefault();
    const res = await axios.post("http://167.99.15.83/api/budgetlines/", {
  ...
  onSubmitDelete = async (id) => {
    const res_p = await axios.post(
      "http://167.99.15.83/api/budgetlines/delete/" + id
    );
    window.location.href = "/project/" + this.props.idProject;
