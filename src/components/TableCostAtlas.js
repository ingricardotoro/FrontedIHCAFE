...
function TableCostAtlas(props) {
    ...
    return ( <
        div >
        <
        RowCardsProjects inicial = { bdg_start_project }
        ...
        tbody > {
            budgetLinesAtlas.map((budgetLinesAtlas) => ( <
                tr key = { budgetLinesAtlas.id } > { /* {suma(budgetLinesAtlas.buddgetstart, budgetLinesAtlas.buddgetfinal,budgetLinesAtlas.balance)}  */ }

                <
                td className = "pro-name" >
                <
                label className = "text-danger" > { budgetLinesAtlas.code } <
                /label> <
                /td> <
                td className = "pro-name" >
                <
                ...
                /td> {
                    budgetLinesAtlas.statu === 'Aprobado' ? ( <
                        ...
                        td >
                        <
                        label className = "text-info" > { ' ' } { formatMoney(budgetLinesAtlas.balance) } <
                        /label> <
                        /td> <
                        />
                    ) : ( <
                ...
                label className = "text-danger" > { ' ' } {
                    formatMoney(
                        moment(budgetLinesAtlas.date_start).format(
                            'DD/MM/YYYY'
                        )
                    )
                } <
                ...
                Eliminar Presupuesto: { ' ' } { budgetLinesAtlas.atlas_account.name } { ' ' } <
                /h4> <
                button type = "button"
                className = "close"
                data - dismiss = "modal"
                ...
                form onSubmit = {
                    () =>
                    onSubmitDelete(budgetLinesAtlas.id)
                } >
                <
                div style = {
                    {
                ...
                Eliminar Este Renglon Presupuestario <
                /button> <
                /div> <
                /form>
                ...
                <
                p class = "modal-title" >
                Descripción: { ' ' } { budgetLinesAtlas.details } <
                /p>

                <
                select onChange = { onchangeSelectAprobar }
                name = "select"
                ...
                input onChange = { onchangeMonto }
                name = "monto"
                type = "text"
                className = "form-control mb-3"
                Value = { budgetLinesAtlas.budgetstart }
                />

                <
                label > Comentarios < /label> <
                textarea onChange = { onchangeComentario }
                name = "comentario"
                ...
                <
                ModalVerFilesAtlas budget_id = { budget_id }
                idProject = { idProject }
                ...
                div class = "modal-header" >
                <
                h4 class = "modal-title" > { budgetLinesAtlas.atlas_account.name } - {
                    formatMoney(
                        budgetLinesAtlas.budgetstart
                    )
                } { ' ' } <
                /h4> <
                button id = {
                    'btnCerrarSF_' + budgetLinesAtlas.id
                }
                type = "button"
                class = "close"
                data - dismiss = "modal"
                ...
                < /input> <
                /div> <
                input value = { budgetLinesAtlas.id }
                name = "budgetlineatlas_id"
                type = "hidden"
                className = "form-control" /
                >
                <
                input value = { budget_id }
                name = "budget_id"
                type = "hidden"
                className = "form-control" /
                >
                <
                input value = { idProject }
                name = "project_id"
                type = "hidden"
                className = "form-control" /
                >
                ...
                div >
                <
                select onChange = { onClickFaseArchivo }
                name = "fase"
                className = "form-control mt-3" >
                <
                option value = "0" >
                ...
                data - dismiss = "modal" >
                Cerrar <
                /button> { /* <button type="button" onClick={ () =>onClickSubirArchivo(budgetLinesAtlas.id )} class="btn btn-primary waves-effect waves-light ">Guardar</button> */ } <
                button onClick = {
                    () =>
                    onClickSubirArchivo(
                        budgetLinesAtlas.id
        ...
        <
        div style = {
            { width: '50%', display: 'inline-block' } } > { /* Select de Resultados Atlas */ } <
        select onChange = { onChanceResultAtlas }
        ...
        /select> { /* FIn del Select de Resultados Atlas */ } <
        /div>

        <
        div style = {
            { width: '50%', display: 'inline-block' } } > { /* Select de Productos Atlas */ } <
        select onChange = { onChanceProductAtlas }
        ...
        /select> { /* FIn del Select de Productos Atlas */ } <
        /div>

        <
        div > { /* Select de Productos Atlas */ } <
        select onChange = { onChanceActivityAtlas }
        ...
        <
        div style = {
            { width: '50%', display: 'inline-block' } } > { /* Select de Cuentas Atlas */ } <
        select onChange = { onChanceAccountAtlas }
        name = "select_account_atlas"
        className = "form-control mt-3" >
        <
        option value = "#" > Seleccione Cuenta Atlas < /option> {
            accounts_atlas.map((account_atlas) => ( <
                option value = { account_atlas.id } > { account_atlas.name } { ' ' } <
                /option>
            ))
        } <
        /select> { /* FIn del Select de Cuentas Atlas */ } <
        /div>

        <
        div style = {
            { width: '50%', display: 'inline-block' } } > { /* Select de Sub-Cuentas Atlas */ } <
        select onChange = { onChanceSubAccountAtlas }
        name = "select_sub_account_atlas"
        className = "form-control mt-3" >
        <
        option value = "#" > Seleccione Cuenta IHCAFE < /option> {
            sub_accounts_atlas.map((sub_account_atlas) => ( <
                option value = { sub_account_atlas.id } > { sub_account_atlas.name } { ' ' } <
                /option>
            ))
        } <
        /select> { /* FIn del Select de Sub-Cuentas Atlas */ } <
        /div>
        ...
        <
        select onChange = { onChanceAccount }
        ...
        div style = {
            { width: '50%', display: 'inline-block' } } >
        <
        select onChange = { onChanceSupplier }
        ...
        /option> {
            suppliers.map((supplier) => ( <
                option value = { supplier.id } > { supplier.contact_name } { ' ' } <
                /option>
            ))
        } <
        /select> <
        ...
        <
        textarea onChange = { onChanceDetails }
