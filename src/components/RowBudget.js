...
export default class RowBudget extends Component {
        ...
        for (let index = 0; index < this.state.budgetLines.length; index++) {
            if (this.state.budgetLines[index].status === 'Solicitado') {
                this.state.total_solicitado =
                    this.state.total_solicitado +
                    this.state.budgetLines[index].buddgetstart;
                ...
                this.state.total_ejecutado +=
                    this.state.budgetLines[index].buddgetfinal;
    ...
    render() {
        ...
        return ( <
            ...
            hr / >
            <
            h4 className = "ml-5 mt-2" > { this.state.nameCategory } < /h4> <
            ...
            tbody > {
                this.state.budgetLines.map((budgetLine) => ( <
                    tr key = { budgetLine.id } > { /* {this.suma(budgetLine.buddgetstart, budgetLine.buddgetfinal,budgetLine.balance)}  */ }
                    ...
                    td >
                    <
                    label className = "text-info" > { ' ' } { this.formatMoney(budgetLine.buddgetstart) } <
                    /label> <
                    /td> <
                    td >
                    <
                    label className = "text-danger" > { ' ' } { this.formatMoney(budgetLine.buddgetfinal) } <
                    /label> <
                    /td> <
                    td >
                    <
                    label className = "text-success" > { ' ' } { this.formatMoney(budgetLine.balance) } <
                    /label> <
                    /td>

                    {
                        budgetLine.status === 'Solicitado' ? ( <
                            ...
                            data - toggle = "modal"
                            data - target = { '#aprobar_' + budgetLine.id } >
                            Decidir <
                            /button>{' '} <
                            /td>
                        ) : ( <
...

                    {
                        budgetLine.status === 'Aprobado' ? ( <
                            ...
                            data - toggle = "modal"
                            data - target = { '#rembolsar_' + budgetLine.id } >
                            Rembolsar <
                            /button> <
                            /td>
                        ) : ( <
                    ...
                    <
                    h4 class = "modal-title" > { budgetLine.name } - { this.formatMoney(budgetLine.buddgetstart) } { ' ' } <
                    /h4> <
                    button type = "button"
                    class = "close"
                    data - dismiss = "modal"
                    ...
                    <
                    select onChange = { this.onchangeSelectAprobar }
                    name = "select"
                    ...
                    Rembolsar { budgetLine.name } - { this.formatMoney(budgetLine.buddgetstart) } { ' ' } <
                    /h4> <
                    button type = "button"
                    class = "close"
                    data - dismiss = "modal"
                    ...
                    /div> <
                    input type = "hidden"
                    name = { 'input_rembolsar_' + budgetLine.id }
                    value = { budgetLine.id }
                    /> <
                    div className = "mt-3" >
                    <
            ...
            td >
            <
            label className = "text-info" > { this.formatMoney(this.state.total_inicial) } <
            /label> <
            /td> <
            td >
            <
            label className = "text-danger" > { this.formatMoney(this.state.total_ejecutado) } <
            /label> <
            /td> <
            td >
            <
            label className = "text-success" > { this.formatMoney(this.state.total_disponible) } <
            /label> <
            /td> <
            td align = "center" >
            <
            ...
            td >
            <
            label > { this.formatMoney(this.state.total_solicitado) } <
            /label> <
            /td> <
            td align = "center" >
            <
