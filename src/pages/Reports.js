...
export default class Reports extends Component {
    ...
    async componentDidMount() {
        ...
        const res = await axios.get('https://167.99.15.83:4000/api/budgets/');
        ...
        /*const res = await axios.get("https://167.99.15.83:4000/api/coins/");
        this.setState({ budgets: res.data.budgets });*/
    }
    ...
    onChangeSelectBudget_Atlas = async(e) => {
        ...
        this.state.projects_atlas.map(async(p_atlas) => {
            ...
            const res = await axios.post(
                'https://167.99.15.83:4000/api/budgetlines/atlas/project/' + p_atlas.id
            );
    ...
    render() {
        return ( <
            ...
            <
            div className = "product-add-modal" > < /div> <
            /div> <
            ...
            td >
            <
            Link to = { 'reports/excel_users' }
            className = "btn btn-block btn-success" >
            { ' ' }
            Generate Excel de Usuarios { ' ' } <
            /Link> <
            /td> <
            /tr> <
            /tbody> <
            ...
            /option> {
                this.state.budgets_atlas.map(
                    (budget) => ( <
                        option value = { budget.id } > { budget.name } { ' ' } <
                        /option>
                    )
                )
            } <
            ...
            <
            label > Moneda < /label> <
            select onChange = { this.onChangeSelectCoin }
            className = "form-control mb-3"
            name = "anio" >
            <
            option value = "1" > LPS < /option> <
            ...
            <
            label > Año < /label> <
            select onChange = { this.onChangeSelectYear }
            className = "form-control mb-3"
            name = "anio" >
            <
            option value = "2020" > 2020 < /option> <
            ...
            td >
            <
            label > Crear Reporte < /label> { /* <button className="btn btn-block btn-primary" onClick={this.reporte_atlas_by_project} type="primary"> Generate PDF </button> */ } <
            Link to = {
