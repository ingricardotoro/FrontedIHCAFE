...
export default class ReportAtlasByProjectid extends Component {
    ...
    async componentDidMount() {
        ...
        const res = await axios.get(
            'https://167.99.15.83:4000/api/budgetlines/atlas/budgets_by_projectid_and_atlasaccountid/' +
        ...
        this.setState({
            ArrayReporteBudgetsByProjectIdByAccountId: res.data.ArrayReporteBudgetsByProjectIdByAccountId,
        });
    }

    SubCaterogia(id) {
        fetch('https://167.99.15.83:4000/api/atlas/find_sub_atlas_category/' + id)
            .then((response) => {
    ...
    render() {
        return ( <
            ...
            /th> <
            th style = {
                { border: '1px solid' } } > Fecha < /th> <
            th style = {
                { border: '1px solid' } } > Valor < /th> <
            /tr> <
            /thead> <
            tbody > {
                this.state.ArrayReporteBudgetsByProjectIdByAccountId.map(
                    (Item) => ( <
                        ...
                        <
                        td style = {
                            { border: '1px solid' } } > { Item.code } { ' ' } <
                        /td> <
                        td style = {
                            { border: '1px solid' } } > { this.SubCaterogia(Item.code_sub_atlas) } <
                        /td> <
                        td style = {
                            { border: '1px solid' } } > { Item.date_start } <
                        /td> <
                        td style = {
                            { border: '1px solid' } } > { this.formatMoney(Item.balance) } <
                        /td> <
                        /tr>
                    )
                )
