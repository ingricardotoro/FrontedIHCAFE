...
export default class TeamMembers extends Component {
    ...
    async componentDidMount() {
        ...
        const res = await axios.get(
            'http://167.99.15.83:4000/api/coins/' + this.props.match.params.id
        );
        this.setState({ coin: res.data.coin });

        //obtenemos las conversiones de esta moneda
        const res2 = await axios.get(
            'http://167.99.15.83:4000/api/conversions/' + this.state.coin.id
        );
    ...
    onSubmitDelete = async(id) => {
        ...
        //window.location.href = "http://167.99.15.83/conversions/" + this.state.coin.id;
    ...
    render() {
        return ( <
            ...
            button type = "button"
            data - toggle = "modal"
            data - target = { '#modal_add_' + this.state.coin.id }
            ...
            tbody > {
                this.state.conversions.map(
                    (conversion) => ( <
                        ...
                        img width = "80px"
                        src = {
                            '../assets/images/teams/conversion.jpg'
                        }
                        className = "img-fluid"
                        alt = "tbl" /
                        >
                        ...
                        td > { conversion.datefinal } < /td> { /* <td> <Link to={'/teamconversions/' + team.id} ><button type="button" className="btn btn-primary waves-effect" >Ver Miembros</button></Link></td> */ } <
                        td >
                        <
                        button type = "button"
                        data - toggle = "modal"
                        data - target = {
                            '#modal_delete_' + conversion.id
                        }
                        ...
                        Eliminar Valor de moneda: { ' ' } { this.state.coin.name } { ' ' } <
                        /h4> <
                        button type = "button"
                        className = "close"
                        data - dismiss = "modal"
            ...
            <
            h4 className = "modal-title" >
            Agergar Nuevo Valor A Moneda: { ' ' } { this.state.coin.name } { ' ' } <
            /h4>
            ...
            placeholder = "Ingrese Valor de Moneda" /
            >
            <
            /div> <
            /div>
            ...
            <
            input required onChange = { this.onChangeEndDate }
            class = "form-control mt-3"
            type = "date"
