...
export default class Users_list extends Component {
    ...
    onChanceTipoUsuario = (e) => {
        this.setState({ tipo_user: e.target.value });
    };

    onSubmit = async(e) => {
        e.preventDefault();
        const resp = await axios.post(
            'http://167.99.15.83:4000/api/users/register', {
        ...
        return ( <
            ...
            tbody > {
                this.state.users.map((user) => ( <
                    ...
                    className = "d-inline-block img-circle "
                    alt = "tbl" /
                    >
                    <
                    /td> <
                    ...
                    <
                    a href = "#"
                    className = "m-r-15 crm-action-edit text-muted" >
                    <
                    ...
                    className = "crm-action-delete text-muted" >
                    <
                    ...
                    Eliminar Usuario: ({ user.name }) - { user.last_name } <
                    /h4> <
                    button type = "button"
                    className = "close"
                    data - dismiss = "modal"
            ...
            div className = "modal-header" >
            <
            h4 className = "modal-title" > Crear Nuevo usuario < /h4> <
            button type = "button"
            className = "close"
            data - dismiss = "modal"
            aria - label = "Close" >
            ...
            <
            select required onChange = { this.onChanceTipoUsuario }
            name = "select"
            ...
            /option> {
                this.state.tipousuarios.map(
                    (tipousuario) => ( <
                        option value = { tipousuario.id } > { tipousuario.tipo_user } <
                        /option>
                    )
                )
            } <
            ...
            div >
            <
            label > Datos personales del usuario < /label> <
            /div> <
            div style = {
                {
                    width: '50%',
            ...
            <
            input required onChange = { this.onChangeUsername }
            class = "form-control "
            type = "text"
