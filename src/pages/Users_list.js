...
export default class Users_list extends Component {
    ...
    async componentDidMount() {
        ...
        const res2 = await axios.get("http://167.99.15.83/api/tipousers/");
    ...
    onSubmit = async (e) => {
        e.preventDefault();
        const resp = await axios.post("http://167.99.15.83/api/users/register", {
    ...
    onSubmitDelete = async (id) => {
        const res_p = await axios.post(
            "http://167.99.15.83/api/users/delete/" + id
        );
        window.location.href = "/users_list";
