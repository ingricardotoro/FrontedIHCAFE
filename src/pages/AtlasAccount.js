...
export default class Atlas extends Component {
    ...
    render() {
        return ( <
            ...
            div className = "page-body" > { /* Hover table card start */ } <
            div className = "card" >
            <
            ...
            tbody > {
                this.state.atlas_accounts.map((result) => ( <
                    ...
                    td > { result.name } < /td> <
                    td style = {
                        { whiteSpace: 'normal' } } > { result.details } <
                    /td> <
...

            {
                this.state.atlas_accounts.map((result2) => ( <
                    AtlasSubAccount className = "mt-3"
                    codeResultado = { result2.id }
                    nameResultado = { result2.name }
                    />
                ))
            } <
